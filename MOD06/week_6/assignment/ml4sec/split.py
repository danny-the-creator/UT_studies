...
def split(
    ...
    raise NotImplementedError(
        "You will need to implement the 'split()' function yourself."
    )
...
def split_numpy(
    ...
    """Perform split function and transform output to numpy arrays.
        ...
        Returns
        -------
        X_train : np.array of shape=(n_flows_train, n_features)
            Datapoints selected for training.

        y_train : np.array of shape=(n_flows_train,)
            Corresponding labels selected for training.

        X_test : np.array of shape=(n_flows_test, n_features)
            Datapoints selected for testing.

        y_test : np.array of shape=(n_flows_test,)
            Corresponding labels selected for testing.
...
def get_split(self, apps_train: List[str], ratio: float) -> Tuple[
    ...
    data, _, _ = self.scale_numpy(self.feature_matrix(self.flows_benign))
    ...
    # Execute split
    X_train, y_train, X_test, y_test = self.split_numpy(
        data       = data,
        labels     = labels,
        apps_train = apps_train,
        apps_test  = apps_test,
...
def test_split(self, verbose: bool = False) -> None:
    ...
    for case, (apps_train, apps_test, ratio, expected) in enumerate(
        ...
        # Execute split
        data_train, labels_train, data_test, labels_test = self.split_numpy(
            data       = data,
            labels     = labels,
            apps_train = apps_train,
            apps_test  = apps_test,
...
def assert_split(
    ...
    # Check if correct number of features is returned
    if data_train.ndim != 2:
        raise ValueError(
            "X_train should be of shape=(n_samples, n_features), i.e. 2 "
            "dimensions, Currently only {} dimension(s) were returned. A likely"
            " cause is that you returned an empty list when no samples were "
            "found, instead try to return a numpy array of "
            "shape=(0, n_features). You can do this with "
            "np.zeros((0, data.shape[1]))."
            .format(data_train.ndim)
        )
    if data_test.ndim != 2:
        raise ValueError(
            "X_test should be of shape=(n_samples, n_features), i.e. 2 "
            "dimensions, Currently only {} dimension(s) were returned. A likely"
            " cause is that you returned an empty list when no samples were "
            "found, instead try to return a numpy array of "
            "shape=(0, n_features). You can do this with "
            "np.zeros((0, data.shape[1]))."
            .format(data_test.ndim)
        )

    # Check if number of features is equal to original
    if  data_train.shape[1] != data_test.shape[1] or\
