...
import ss.project.game.ai.StrategyLvlEasy;
...
public class ClientConnection extends SocketConnection {
    ...
    protected void handleMessage(String message) {
        ...
        switch (commands[0]) {
            ...
            case Protocol.ALREADYLOGGEDIN -> {
                System.out.println("The user with that name already exists, please try another username");
                System.out.println("You can see the list of already existing names by typing list");
                this.name = null;
            }
            case Protocol.MOVE -> {
                client.getGame().doMove(new MoveDnB(client.getGame().getTurn().getColour(), Integer.parseInt(commands[1])));
                System.out.println(client.getGame().toString());
            }
            case Protocol.GAMEOVER -> {
                if (message.split(Protocol.SEPARATOR)[1].equals(Protocol.DISCONNECT)) {
                    System.out.println("Your opponent has disconnected from the game. You won!");
                } else if (message.split(Protocol.SEPARATOR)[1].equals(Protocol.VICTORY)) {
                    System.out.println("The player " + (message.split(Protocol.SEPARATOR)[2] + " won the game!"));
                } else if (message.split(Protocol.SEPARATOR)[1].equals(Protocol.DRAW)) {
    ...
    public void sendMove(String move) {
        super.sendMessage(Protocol.MOVE+Protocol.SEPARATOR+move);
    }
    /**
