 ...
 * It includes functionality for managing players, the game board, turns, moves, and checking for the game's end.
 */
public class GameDnB implements Game{
    ...
    public void doMove(Move move) {
        if (board.setField(move.getLocation(), move.getColour())){// paints the chose line and checks if no boxes were painted
            if (getTurn() == player1){
                current = player2;
            } else {
                current = player1;
    ...
    public String toString(){
        return board.EXAMPLE+"\n\n" +board.createBoard() +"\n"+"Current Player: "+ current.getColour()+current.getName()+ Components.RESET;
    }
    /**
