...
import project.client.SocketConnection;
...
public class ServerConnection extends SocketConnection {
     ...
     * parsing and processing user commands and chat messages.
     *
    ...
    public void handleMessage(String message) {
        ...
        switch (command) {
            case Protocol.MOVE -> clientHandler.recieveCHCommand(message);
            case Protocol.LIST -> clientHandler.recieveCHCommand(action.get(0));
            case Protocol.QUEUE -> clientHandler.recieveCHCommand(command);
            case Protocol.HELLO -> {
                clientHandler.recieveCHCommand(action.get(0));
                System.out.println(
                        Protocol.HELLO + Protocol.SEPARATOR + "Welcome to server by Danil and Mira");
            }
            case Protocol.LOGIN -> {
     ...
     */
    public void sendSCCommand(String message) {
        // there was a String name as a parameter, removed as we don't need it
