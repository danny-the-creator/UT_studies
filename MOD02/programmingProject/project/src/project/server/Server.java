...
public class Server extends SocketServer {
     ...
     * @throws IOException if the server socket cannot be created, for example,
        ...
        if (clientHandler.getHello()) {
            switch (action.get(0)) {
                case Protocol.MOVE -> {
                    ...
                    if (!clientHandler.game.getTurn().getName()
                            .equals(clientHandler.getUsername())) {
                        // checks if the player who sent the message is the current player and
                        // if not sends back and error.
                        clientHandler.sendCHCommand(
                                Protocol.ERROR + Protocol.SEPARATOR + "WAIT YOUR TURN!");
                        break;
                    }
                    try {
                        ...
                        clientHandler.sendCHCommand(
                                Protocol.ERROR + Protocol.SEPARATOR + "WRONG MOVE! TRY AGAIN!");
                        break;
                    }
                    clientHandler.game.doMove(move);
                    clientHandler.sendCHCommand(Protocol.MOVE + Protocol.SEPARATOR + action.get(1));
                    clientHandler.getPartner()
                            .sendCHCommand(Protocol.MOVE + Protocol.SEPARATOR + action.get(1));
                    ...
                    if (winner == null) {
                        clientHandler.sendCHCommand(
                                Protocol.GAMEOVER + Protocol.SEPARATOR + Protocol.DRAW);
                ...
                case Protocol.QUEUE -> {
                    synchronized (queue) {
                        ...
                        if (queue.size() >= 2) {
                            ...
                            DnBTUI tui = new DnBTUI(new GameDnB(), player1,
                                                    player2);   // the constructor sets the players
                ...
                case Protocol.LIST -> {
                    StringBuilder names = new StringBuilder(Protocol.LIST);
                    for (ClientHandler client : clients) {
                        if (client.getUsername() != null) {    // sends all the players, who have usernames
                            names.append(Protocol.SEPARATOR).append(client.getUsername());
                        }
                    }
                    clientHandler.sendCHCommand(names.toString());
                }
            }
        }
    }
