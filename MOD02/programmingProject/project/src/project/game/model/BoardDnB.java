...
public class BoardDnB {
    /**
     * Dimension of the board, i.e., if set to 5, the board has 5 rows,
     * 5 columns and 60 lines (from 0 to 59).
     */
    public static final int DIM = 3;
    private final String POINT = "*";

    /**
     * After constructor is called, here NumberBoard will be stored,
     * which is going to show indexes of all lines.
     */
    ...
    public String createBoard() {
        int num = 0;
        StringBuilder board = new StringBuilder(POINT);
        for (int i = 0; i < DIM; i++) {

            for (int j = 0; j < DIM; j++) {
                board.append(LINES.get(num)).append(POINT);
                num++;
            }
            board.append("\n");
            for (int j = 0; j < DIM; j++) {
                board.append(LINES.get(num)).append(BOXES.get(i * DIM + j));
                num++;
            }
            board.append(LINES.get(num)).append("\n").append(POINT);
            num++;
        }
        for (int j = 0; j < DIM; j++) {
            board.append(LINES.get(num)).append(POINT);
    ...
    public String createNumberBoard() {
        ...
        for (int i = 0; i < DIM; i++) {
            ...
            for (int j = 0; j < DIM; j++) {
                board.append(NUMBERS.get(num)).append(POINT);
                num++;
            }
            board.append("\n");
            for (int j = 0; j < DIM; j++) {
                board.append(NUMBERS.get(num)).append(Components.EMPTY);
                num++;
            }
            board.append(NUMBERS.get(num)).append("\n").append(POINT);
            num++;
        }
        for (int j = 0; j < DIM; j++) {
            board.append(NUMBERS.get(num)).append(POINT);
    ...
    public BoardDnB() {
        ...
        for (int i = 0; i < LINES.size(); i++) {
            // this loop creates a list of indexes and insures they look good on the board
    ...
    public BoardDnB deepCopy() {
        ...
        for (int i = 0; i < LINES.size(); i++) { board.LINES.set(i, LINES.get(i)); }
        return board;
    }
    ...
    public String findWinner() {
        ...
        for (String box : BOXES) {
            // for each box the loop checks who have taken it and add a point to this player
    ...
    public void finishBox(int i, String colour) {
        ...
        if (getLine(i).contains(Components.HORIZONTAL)) {
            ...
            if (checkSides(sides)) {
                ...
                BOXES.set(i / (2 * DIM + 1) * DIM + i % (2 * DIM + 1),
                          colour + Components.TAKEN + Components.RESET);
            }
            ...
            if (checkSides(sides)) {
                ...
                BOXES.set((i / (2 * DIM + 1) * DIM + i % (2 * DIM + 1) - DIM),
                          colour + Components.TAKEN + Components.RESET);
            }
            ...
            if (getLine(i + 1).contains(Components.VERTICAL) && getLine(i - DIM).contains(
                ...
                BOXES.set(((i - DIM) / (2 * DIM + 1) * DIM + (i - DIM) % (2 * DIM + 1)),
                          colour + Components.TAKEN + Components.RESET);
            }
            ...
            if (getLine(i - 1).contains(Components.VERTICAL) && getLine(i - DIM - 1).contains(
                ...
                BOXES.set(((i - DIM - 1) / (2 * DIM + 1) * DIM + (i - DIM - 1) % (2 * DIM + 1)),
                          colour + Components.TAKEN + Components.RESET);
            }
        }
    }
    ...
    public void reset() {
        ...
        for (int i = 0; i < LINES.size(); i++) { LINES.set(i, EmptyLines.get(i)); }
    }
