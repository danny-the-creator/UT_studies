...
# All seq are stored in halfwords now and we need to find all combinations
  addu $t0, $t2, $t2  # offset - $t0 (number of bytes in one user number)	
  move $t2, $t0
  move $t1, $zero
# $t6 - len array-
  ...
  halfWLoop:
  addu $t4, $t1, $s0
  addu $t4, $t4, $t8	#-		t8 - offset for another halfword of user's number
  lh $t4, 0($t4)  # !    # s0 - is the beginning of an array
...
bne $t4, $zero, bit_loop    # Exit loop if $t5 becomes 0 (all non-zero bits processed)
    ...
    addiu $t8, $t8, 2	# move to the next half word
    bne $t8,$t0, halfWLoop # when the program reaches the last half word of user's input, break the loop
#------------------------------------------------------------------------------    
    
move $t3, $t6
Loop_1:				# It needed to break the jump into 2 smaller ones, for number 3
bne $t3, $t6, Loop		#						(it allows us to store "imm" in 5 bytes)
...
#Print the separator
    lh $a0, sep_addr              # Load the address of the separator string into $a0
    addi $v0, $zero, 2                     # Syscall for print string
